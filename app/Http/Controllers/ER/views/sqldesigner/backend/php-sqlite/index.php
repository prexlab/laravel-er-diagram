<?php
    ...
    switch ($a) {
        ...
        case 'save':
            ...
            if ($result[0]["c"] > 0) {
                $result = $pdo->prepare("UPDATE ".TABLE." SET data = ? WHERE keyword = ?")->execute([$data, $keyword]);
            } else {
                $result = $pdo->prepare("INSERT INTO ".TABLE." (keyword, data) VALUES (?, ?)")->execute([$keyword, $data]);
            }
