    ...
    if ($GLOBALS['calculate_last_insert_id']) {
        if (startsWith($cubrid_query, "insert", false)) {
            ...
            if (strlen($last_insert_id_column) > 0) {
                $GLOBALS['cubrid_last_insert_id'] = cubrid_get_value(
            cubrid_last_insert_id_query($last_insert_id_column),
                                                           $link_identifier
        );
            }
        } else {
...
{
    $id_result
       = cubrid_mysql_query_native(
           "SELECT class_name FROM db_class WHERE class_name='".$tablename."'",
                                  $link_identifier
       );
