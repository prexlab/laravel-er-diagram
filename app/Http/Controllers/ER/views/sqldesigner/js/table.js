...
SQL.Table.prototype.select = function() { 
	...
	jQuery('path').addClass('unselected');
  var rs = this.getRelations();
  for (var i=0;i<rs.length;i++) {
    rs[i].addClass('selected');
    rs[i].removeClass('unselected');
  }
...
SQL.Table.prototype.deselect = function() { 
  ...
  jQuery('path').removeClass('unselected');
  var rs = this.getRelations();
  for (var i=0;i<rs.length;i++) { rs[i].removeClass('selected'); }
