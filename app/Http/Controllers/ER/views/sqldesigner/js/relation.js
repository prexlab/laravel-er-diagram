...
SQL.Relation = function(owner, row1, row2) {
    ...
    switch(this.row2.data.comment){
      case 'hasMany':

        console.log([path, this.row1.data, this.row1.owner.x, this.row2.data, this.row2.owner.x]);

        if(this.row1.owner.x < this.row2.owner.x
          || (this.row1.owner.x == this.row2.owner.x && this.row1.owner.y < this.row2.owner.y)){
...
SQL.Relation.prototype.addClass = function(cls){
  jQuery(this.dom[0]).addClass(cls);
};

SQL.Relation.prototype.removeClass = function(cls){
  jQuery(this.dom[0]).removeClass(cls);
}
